...
filter_measured_modes() {
  local filtered=""
  local skipped=""
  local mode
  for mode in $1; do
    case "${mode}" in
      no_prefetch|direct_prefetch|dynamo_priority_hints|e2e_priority_hints|oracle_prefetch)
        filtered="${filtered} ${mode}"
        ;;
      projected_hardware|projected_hardware_bypass|hardware_bypass)
        skipped="${skipped} ${mode}"
        ;;
      *)
        echo "ERROR: unsupported Milestone 27 mode '${mode}'" >&2
        echo "Measured modes: no_prefetch direct_prefetch dynamo_priority_hints e2e_priority_hints oracle_prefetch" >&2
        echo "Projected hardware rows are generated by the report builder; do not run them as SGLang workloads." >&2
        exit 2
        ;;
    esac
  done
  if [[ -n "${skipped}" ]]; then
    echo "Skipping report-only projected mode(s):${skipped}"
  fi
  # shellcheck disable=SC2086
  echo ${filtered}
}

EXEC_MODES="$(filter_measured_modes "${MODES}")"
if [[ -z "${EXEC_MODES}" ]]; then
  echo "ERROR: no measured SGLang modes left to run after filtering MODES='${MODES}'" >&2
  exit 2
fi

mode_count="$(count_words "${EXEC_MODES}")"
