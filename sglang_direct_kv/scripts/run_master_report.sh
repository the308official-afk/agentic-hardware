...
run_controlled() {
  ...
  local knob
  for knob in MAX_PAIRS MODES TOOL_WAIT_LIST_MS FILLER_LIST FILLER_PROMPT_TOKENS TARGET_PROMPT_TOKENS SYNTHETIC_PROMPT_TOKENS SYNTHETIC_REPLAY_SUFFIX_TOKENS FILLER_DIVERGE_EARLY REQUEST_CONCURRENCY MAX_TOTAL_TOKENS HICACHE_SIZE_GB MEM_FRACTION_STATIC AGENTIC_KV_TRACE_KV_POOL; do
    if [[ -n "${!knob+x}" ]]; then
      env_args+=("${knob}=${!knob}")
    fi
  done
...
run_multi_session() {
  ...
  local knob
  for knob in SESSION_COUNT MODES ARRIVAL_SHAPE ARRIVAL_GAP_MS ARRIVAL_GAP_RANGE_MS BURST_SIZE BURST_GAP_MS TOOL_WAIT_LIST_MS TOOL_WAIT_JITTER_MS PREFETCH_TIMING HINT_DELAY_MS PREFETCH_LEAD_MS PRIORITY_PREFETCH_WINDOW_MS PRIORITY_POST_PREFETCH_QUIET_MS BACKGROUND_FILLERS_PER_SESSION FILLER_PROMPT_TOKENS TARGET_PROMPT_TOKENS SYNTHETIC_PROMPT_TOKENS SYNTHETIC_REPLAY_SUFFIX_TOKENS REQUEST_CONCURRENCY MAX_TOTAL_TOKENS HICACHE_SIZE_GB MEM_FRACTION_STATIC AGENTIC_KV_TRACE_KV_POOL; do
    if [[ -n "${!knob+x}" ]]; then
      env_args+=("${knob}=${!knob}")
    fi
  done
