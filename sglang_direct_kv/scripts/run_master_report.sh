...
export AGENTIC_RUNTIME_TELEMETRY AGENTIC_RUNTIME_TELEMETRY_BACKEND
...
write_manifest() {
  ...
  AGENTIC_RUNTIME_TELEMETRY_VALUE="${AGENTIC_RUNTIME_TELEMETRY:-}" \
  AGENTIC_RUNTIME_TELEMETRY_BACKEND_VALUE="${AGENTIC_RUNTIME_TELEMETRY_BACKEND:-}" \
  ...
  SYNC_REPLAY_AFTER_INITIALS_VALUE="${SYNC_REPLAY_AFTER_INITIALS:-}" \
...
manifest = {
    ...
    "pressure_knobs": {
        ...
        "agentic_runtime_telemetry": os.environ.get("AGENTIC_RUNTIME_TELEMETRY_VALUE", ""),
        "agentic_runtime_telemetry_backend": os.environ.get("AGENTIC_RUNTIME_TELEMETRY_BACKEND_VALUE", ""),
        ...
        "sync_replay_after_initials": os.environ.get("SYNC_REPLAY_AFTER_INITIALS_VALUE", ""),
...
SYNC_REPLAY_AFTER_INITIALS=${SYNC_REPLAY_AFTER_INITIALS:-}
...
run_multi_session() {
  ...
  local knob
  for knob in SESSION_COUNT MODES ARRIVAL_SHAPE ARRIVAL_GAP_MS ARRIVAL_GAP_RANGE_MS BURST_SIZE BURST_GAP_MS TOOL_WAIT_LIST_MS TOOL_WAIT_JITTER_MS SYNC_REPLAY_AFTER_INITIALS PREFETCH_TIMING HINT_DELAY_MS PREFETCH_LEAD_MS PRIORITY_PREFETCH_WINDOW_MS PRIORITY_POST_PREFETCH_QUIET_MS DEADLINE_RESERVE_WINDOW_MS BACKGROUND_FILLERS_PER_SESSION FILLER_PROMPT_TOKENS TARGET_PROMPT_TOKENS SYNTHETIC_PROMPT_TOKENS SYNTHETIC_REPLAY_SUFFIX_TOKENS REQUEST_CONCURRENCY MAX_TOTAL_TOKENS HICACHE_SIZE_GB MEM_FRACTION_STATIC AGENTIC_KV_TRACE_KV_POOL AGENTIC_RUNTIME_TELEMETRY AGENTIC_RUNTIME_TELEMETRY_BACKEND AGENTIC_KV_GPU_UTIL_SAMPLER GPU_UTIL_SAMPLE_INTERVAL_MS DYNAMO_HIGH_PRIORITY DYNAMO_NORMAL_PRIORITY DYNAMO_LOW_PRIORITY DYNAMO_RADIX_EVICTION_POLICY DYNAMO_SCHEDULE_POLICY; do
    if [[ -n "${!knob+x}" ]]; then
      env_args+=("${knob}=${!knob}")
    fi
  done
