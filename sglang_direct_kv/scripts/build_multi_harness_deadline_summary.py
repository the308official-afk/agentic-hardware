...
CHART_SIGNAL_BUCKETS = {
    ...
    "controller_priority_demotion_admission_soft": {
        "label": "Controller Admission Soft",
        "description": "Controller raises replay priority and holds background admission only during the final quarter of the tool-wait window",
        "color": "#0284c7",
        "modes": {"controller_priority_demotion_admission_soft"},
    },
    "controller_priority_demotion_admission_medium": {
        "label": "Controller Admission Medium",
        "description": "Controller raises replay priority and holds background admission during the final half of the tool-wait window",
        "color": "#0369a1",
        "modes": {"controller_priority_demotion_admission_medium"},
    },
    "controller_priority_demotion_admission_hard": {
        "label": "Controller Admission Hard",
        "description": "Controller raises replay priority and holds background admission for the full tool-wait window",
        "color": "#1d4ed8",
    ...
    "controller_priority_demotion_admission_shorthand": {
        "label": "Controller Shorthand",
        "description": "Controller priority, demotion, and admission plus request-local prompt shorthand when it proves net token savings",
        "color": "#be185d",
    ...
    for pressure_index, pressure in enumerate(pressures):
        ...
        for harness_index, harness in enumerate(harnesses):
            ...
            for delta_index, bucket in enumerate(comparison_buckets):
                ...
                delta_palette = COST_ACCOUNTING_DELTA_COLORS.get(bucket, {})
                delta_color = (
                    COST_ACCOUNTING_DELTA_UNKNOWN
                    if delta is None
                    else str(delta_palette.get("better") or COST_ACCOUNTING_DELTA_BETTER)
                    if delta >= 0
                ...
                delta_label_name = (
                    ...
                    if bucket == "controller_priority_demotion_admission_soft"
                    else "PDA medium"
                ...
                delta_title = (
                    ...
                    f"(baseline total - {chart_signal_label(bucket).lower()} total) "
                ...
                if delta is None:
                    ...
                    lines.append(
                        f'<line x1="{delta_cx - 5:.1f}" x2="{delta_cx + 5:.1f}" y1="{zero_y:.1f}" y2="{zero_y:.1f}" '
                        f'stroke="{delta_color}" stroke-width="2.2" opacity="0.8"/>'
                    )
                    lines.append(svg_text_label("n/a", delta_cx, delta_label_y, delta_color))
                else:
                    delta_y = y_pos(delta)
                    ...
                    lines.append(
                        f'<rect x="{delta_cx - 5:.1f}" y="{bar_y:.1f}" width="10" height="{bar_h:.1f}" '
