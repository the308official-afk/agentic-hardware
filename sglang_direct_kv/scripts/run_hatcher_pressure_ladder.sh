...
# Full Hatcher pressure ladder.
#
# This is the manager-facing ladder: six bundled pressure levels across the two
# modes we currently care about, without running a full Cartesian sweep.
export REPORT_LABEL="${REPORT_LABEL:-hatcher_pressure_ladder_$(date +%Y%m%d_%H%M%S)}"
...
export MAX_TIMELINE_GAPS="${MAX_TIMELINE_GAPS:-96}"
...
write_run_config() {
  {
    ...
    echo "MODEL=${MODEL}"
    echo "EXPERIMENT_KIND=pressure_ladder"
    ...
    echo "MODES=${MODES}"
    echo "PRESSURE_LADDER_LEVELS=P0 Control; P1 Short Wait; P2 Large KV; P3 Queue Pressure; P4 KV Pool Pressure; P5 Boss Queue"
    ...
    echo "P1_SHORT_WAIT=tool_wait_ms=50,target_prompt_tokens=1024,fillers=0,request_concurrency=1,urgent_agents=1,max_total_tokens=8192"
    echo "P2_LARGE_KV=tool_wait_ms=500,target_prompt_tokens=4096,fillers=0,request_concurrency=1,urgent_agents=1,max_total_tokens=12288"
    echo "P3_QUEUE_PRESSURE=tool_wait_ms=50,target_prompt_tokens=4096,fillers=32,request_concurrency=8,urgent_agents=1,max_total_tokens=12288"
    echo "P4_KV_POOL_PRESSURE=tool_wait_ms=50,target_prompt_tokens=4096,fillers=48,filler_prompt_tokens=2048,request_concurrency=8,urgent_agents=1,max_total_tokens=12288"
    echo "P5_BOSS_QUEUE=tool_wait_ms=50,target_prompt_tokens=4096,background_fillers_per_session=4,filler_prompt_tokens=2048,request_concurrency=12,urgent_agents=8,arrival_shape=burst,sync_replay_after_initials=1,max_total_tokens=12288"
...
run_boss_queue_level() {
  echo
  echo "==== Pressure level p5_boss_queue: urgent burst plus filler pressure ===="
  env \
    PRESSURE_LEVEL="p5_boss_queue" \
    RESULT_ROOT="${RUN_ROOT}" \
    LATEST_REPORT_ROOT="${RUN_ROOT}/_latest_scratch" \
    WORKLOAD_SOURCE="${WORKLOAD_SOURCE}" \
    MODES="${MODES}" \
...
build_final_report() {
  echo
  echo "==== Building Replay Deadline Pressure Chart report ===="
  write_run_config
  collect_run_environment
...
manifest = {
    ...
    "experiment_kind": "pressure_ladder",
    ...
    "script": "scripts/run_hatcher_pressure_ladder.sh",
    "pressure_profile": "p0_to_p5_full",
    "chart_title": "Replay Deadline Pressure Chart",
    "pressure_knobs": {
        "modes": "no_prefetch e2e_priority_hints",
        "pressure_ladder_levels": "P0 Control; P1 Short Wait; P2 Large KV; P3 Queue Pressure; P4 KV Pool Pressure; P5 Boss Queue",
        ...
        "p1_short_wait": "tool_wait_ms=50,target_prompt_tokens=1024,fillers=0,request_concurrency=1,urgent_agents=1,max_total_tokens=8192",
        "p2_large_kv": "tool_wait_ms=500,target_prompt_tokens=4096,fillers=0,request_concurrency=1,urgent_agents=1,max_total_tokens=12288",
        "p3_queue_pressure": "tool_wait_ms=50,target_prompt_tokens=4096,fillers=32,request_concurrency=8,urgent_agents=1,max_total_tokens=12288",
        "p4_kv_pool_pressure": "tool_wait_ms=50,target_prompt_tokens=4096,fillers=48,filler_prompt_tokens=2048,request_concurrency=8,urgent_agents=1,max_total_tokens=12288",
        "p5_boss_queue": "tool_wait_ms=50,target_prompt_tokens=4096,background_fillers_per_session=4,filler_prompt_tokens=2048,request_concurrency=12,urgent_agents=8,arrival_shape=burst,sync_replay_after_initials=1,max_total_tokens=12288",
    },
}
...
echo "Replay Deadline Pressure Chart"
echo "MODEL=${MODEL}"
echo "REPORT_LABEL=${REPORT_LABEL}"
echo "RUN_ROOT=${RUN_ROOT}"
echo "REPORT_DIR=${REPORT_DIR}"
echo "MODES=${MODES}"
echo "Levels: P0 Control, P1 Short Wait, P2 Large KV, P3 Queue Pressure, P4 KV Pool Pressure, P5 Boss Queue"

start_gpu_util_sampler
run_controlled_level "p0_control" "500" "1024" "0" "1" "768" "8192"
run_controlled_level "p1_mild" "50" "1024" "0" "1" "768" "8192"
run_controlled_level "p2_medium" "500" "4096" "0" "1" "768" "12288"
run_controlled_level "p3_high" "50" "4096" "32" "8" "1536" "12288"
run_controlled_level "p4_cliff" "50" "4096" "48" "8" "2048" "12288"
run_boss_queue_level
stop_gpu_util_sampler
build_final_report
